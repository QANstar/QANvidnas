...
- [x] 1.1 初始化 Go 项目：`server/` 目录，Go module，依赖管理（gin/chi, go-sqlite3, jwt-go, fsnotify, ffmpeg-go, yaml）
- [x] 1.2 初始化 React 前端：`client/` 目录，Vite + React 18 + TypeScript + React Router，粉蓝主题 CSS 变量
- [x] 1.3 配置 Makefile：`make dev`, `make build`, `make docker`, `make apk-tv`, `make apk-mobile`
- [x] 1.4 创建 `config.example.yaml` 样例配置文件，`.gitignore` 排除 `config.yaml` 和构建产物

## 2. 数据库与配置

- [x] 2.1 创建 SQLite 数据库 schema（users, invite_codes, media, tags, media_tags, playlists, playlist_items, scan_folders 表）
- [x] 2.2 实现配置加载：读取 `config.yaml`，配置热加载（fsnotify 监听配置文件变更）
- [x] 2.3 创建 FTS5 全文搜索虚拟表（覆盖 media.title, media.description, tags.name）
- [x] 2.4 实现数据库迁移机制（版本化 schema，自动升级）

## 3. 用户认证系统

- [ ] 3.1 实现管理员初始化 API（`POST /api/auth/setup`）：首次检测 + 创建管理员 + 关闭接口
- [ ] 3.2 实现注册码注册 API（`POST /api/auth/register`）：验证注册码 → 创建用户 → 计数递增
- [x] 3.3 实现 JWT 登录 API（`POST /api/auth/login`）：用户名密码验证 → 签发 7 天有效 Token
- [x] 3.4 实现 JWT 认证中间件：保护所有 `/api/*` 路由（除 auth 和 setup）
...
- [x] 4.3 实现 fsnotify 实时监听：文件增删改事件 → 防抖 5s → 增量处理
- [x] 4.4 实现定时轮询降级：每 5 分钟检查 mtime → 发现新文件加入队列
- [x] 4.5 实现手动扫描 API（`POST /api/media/scan`）：触发全量重扫 + 进度推送（WebSocket）
...
- [x] 5.3 实现异步生成队列：封面和雪碧图后台生成，不阻塞扫描主流程
- [x] 5.4 实现默认占位图：视频（播放图标）、音频（音符图标）作为 SVG 内嵌

## 6. 媒体管理 API

- [x] 6.1 实现媒体列表 API（`GET /api/media`）：分页 + 排序（名称/日期/时长）+ 类型筛选
- [ ] 6.2 实现媒体详情 API（`GET /api/media/:id`）
- [ ] 6.3 实现媒体元数据编辑 API（`PUT /api/media/:id`）：title, description, tags
- [ ] 6.4 实现自定义封面上传 API（`POST /api/media/:id/cover`）：接收图片 → 覆盖自动封面
- [ ] 6.5 实现标签 CRUD API（`GET/POST/DELETE /api/tags`）

## 7. 搜索功能

- [ ] 7.1 实现全文搜索 API（`GET /api/search`）：FTS5 查询 → 拼音兼容 + 相关度排序
- [ ] 7.2 实现标签筛选 API：单标签 / 多标签 AND 逻辑
- [ ] 7.3 实现组合筛选：搜索关键词 + 标签 + 类型 三条件交集

## 8. 媒体流服务

- [ ] 8.1 实现视频流 Handler（`GET /api/stream/video/:id`）：HTTP Range 支持 → 206 Partial Content
- [ ] 8.2 实现音频流 Handler（`GET /api/stream/audio/:id`）：HTTP Range 支持
- [ ] 8.3 实现封面图服务（`GET /api/stream/cover/:id`）：返回封面文件
- [x] 8.4 实现路径安全校验：防止路径遍历攻击
- [x] 8.5 实现 MIME 类型自动识别（根据文件扩展名设置 Content-Type）

## 9. 播放列表管理

- [ ] 9.1 实现播放列表 CRUD API：创建（指定文件夹路径 → 递归扫描子文件夹）→ 查看 → 删除
...
- [ ] 10.1 实现文件上传 API（`POST /api/upload`）：Multipart 接收 → 分块写入 → 进度反馈
- [x] 10.2 实现上传限制：格式校验 + 大小上限 10GB + 目标目录校验
- [x] 10.3 实现上传后自动触发扫描：文件写入完成后加入扫描队列

## 11. 前端：核心框架

- [ ] 11.1 配置 Vite + React Router，响应式布局（手机/平板/PC 三档断点）
- [x] 11.2 实现粉蓝主题色系统：CSS 自定义属性，深色背景 + 粉蓝高亮，电视端纯暗色
- [x] 11.3 实现 API 客户端封装：axios/fetch + JWT token 管理 + 401 自动跳转登录
...
- [x] 21.1 编写 Dockerfile：多阶段构建（Go 编译 + FFmpeg 安装 + React 构建 + embed）
- [x] 21.2 编写 docker-compose.yml：端口映射 + 媒体文件夹挂载 + 配置/数据目录挂载
- [ ] 21.3 构建 Docker 镜像并验证在飞牛 NAS 上部署运行
- [x] 21.4 编写部署文档：docker-compose up 一条命令部署
