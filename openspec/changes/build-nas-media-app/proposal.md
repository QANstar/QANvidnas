...
NAS 上存了大量音视频文件，但没有一个方便的多端管理播放工具。现有的开源方案（Jellyfin/Emby）太重、配置复杂，且界面不够现代活泼。QANvidnas 要做的是一个轻量、美观、开箱即用的 NAS 媒体中心——部署简单（一个 Docker 容器），支持手机/电视/PC 三端，用注册码控制访问。

## What Changes

- 新增 Go 后端服务，提供 REST API、媒体流、文件监听、缩略图生成等全部服务端能力
- 新增 React 前端（Web），支持 PC 浏览器和手机浏览器访问
...
- `media-scan`: 媒体扫描与索引。管理员指定 NAS 文件夹路径，自动(fsnotify+轮询)和手动扫描，提取视频/音频文件元数据。
- `media-metadata`: 媒体元数据管理。编辑名称、描述、标签，自定义封面上传，自动封面生成(FFmpeg 取帧+黑场检测)。
- `media-search`: 媒体搜索与筛选。按标签筛选，全文搜索(文件名/标签/描述)，SQLite FTS5 引擎。
- `media-stream`: 媒体流服务。HTTP Range 请求支持视频/音频流，所有文件通过 Go 后端代理(需认证)。
...
- `config-management`: 配置管理。YAML 配置文件(注册码、文件夹路径等)，样例文件提交 git，真实配置 gitignore。
...
- **依赖**：Go 1.22+, React 18+, SQLite (mattn/go-sqlite3), FFmpeg, Capacitor 6, ExoPlayer, fsnotify
- **部署**：Docker 单容器部署在飞牛 NAS (x86)，HTTP 协议局域网访问，NAS 媒体文件夹通过 Docker volume 挂载
- **构建产物**：Go 二进制(内嵌 React SPA) + Android 手机 APK + Android TV APK
